...
<head>
    ...
    <style>
        ...
        *,body{font-family:'Inter','Segoe UI',sans-serif;}
        ...
        .premium-table tbody tr:hover{background:#FAFBFF;}
        ...
        /* Barangay sync panel */
        .sync-panel{background:white;border-radius:16px;border:2px solid #10B981;padding:20px 24px;margin-bottom:24px;}
        .sync-panel-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:12px;}
        .sync-title{font-size:.95rem;font-weight:800;color:#065F46;display:flex;align-items:center;gap:8px;}
        .sync-badge{background:#D1FAE5;color:#065F46;border:1px solid #6EE7B7;border-radius:20px;padding:3px 12px;font-size:.75rem;font-weight:700;}
        .btn-sync{background:linear-gradient(135deg,#10B981,#059669);color:white;border:none;border-radius:30px;padding:9px 22px;font-weight:700;font-size:.87rem;cursor:pointer;transition:all .3s;}
        .btn-sync:hover{transform:translateY(-2px);box-shadow:0 6px 16px rgba(16,185,129,.35);}
        .sync-table{width:100%;border-collapse:collapse;font-size:.83rem;}
        .sync-table th{background:#F0FDF4;color:#065F46;padding:8px 14px;font-weight:700;text-transform:uppercase;font-size:.73rem;letter-spacing:.06em;border-bottom:2px solid #6EE7B7;}
        .sync-table td{padding:8px 14px;border-bottom:1px solid #D1FAE5;color:#334155;}
        .sync-table tr:last-child td{border-bottom:none;}
        .bgy-badge{display:inline-block;background:#D1FAE5;color:#065F46;border-radius:6px;padding:1px 8px;font-size:.7rem;font-weight:700;margin-left:6px;vertical-align:middle;}
        .warn-badge{display:inline-block;background:#FEF3C7;color:#92400E;border-radius:6px;padding:1px 8px;font-size:.7rem;font-weight:700;margin-left:6px;vertical-align:middle;}
        ...
        .chart-title{font-size:.95rem;font-weight:800;color:var(--primary-blue);margin-bottom:4px;}
        .chart-subtitle{font-size:.82rem;color:#94a3b8;margin-bottom:16px;}
        ...
        .alert-success-custom{background:#EFF6FF;color:#1D4ED8;border:none;border-left:4px solid #1D4ED8;border-radius:12px;padding:12px 18px;margin-bottom:20px;}
        ...
        <div class="container py-5">
            ...
            <div class="section-tab active" id="tab-records">
                ...
                <div class="sync-panel">
                    ...
                    <p style="font-size:.82rem;color:#6B7280;margin-bottom:14px;">These are the <strong>live sums</strong> computed from all barangay records. Click <em>Sync</em> to save them as the official beneficiary counts in the programs table.</p>
                    ...
                    <div class="table-responsive">
                        <table class="premium-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                        ...
                                        @if(in_array($program->program_type, ['AICS','PWD_Assistance','Solo_Parent']))
                                            ...
                                            @if(isset($barangayLookup[$bKey]))
                                                ...
                                                @elseif($bTotal !== null && $bTotal != $program->beneficiary_count)
                                                    <span class="warn-badge" title="Barangay total: {{ number_format($bTotal) }}">⚠ Bgy: {{ number_format($bTotal) }}</span>
                                                @endif
                                            @endif
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            ...
                                            <button class="btn-action-delete" onclick="archiveProgram({{ $program->id }}, '{{ addslashes(str_replace('_',' ',$program->program_type)) }}', '{{ $program->municipality }}')">Archive</button>
                                        </div>
                                    </td>
                                </tr>
                                            ...
                                            <form method="POST" action="{{ route('superadmin.data.programs.update', $program->id) }}">
                                                ...
                                                <div class="modal-body p-4">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="number" name="beneficiary_count" class="form-control" value="{{ $program->beneficiary_count }}" required min="0">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label">Year</label>
                                ...
                                @empty
                                <tr><td colspan="6" style="text-align:center;padding:48px;color:#94a3b8;">No program records found. Add one using the button above.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
