...
<head>
    ...
    <style>
        ...
        .box { background: #ecfdf5; border-left: 4px solid #16a34a; border-radius: 10px; padding: 14px 16px; font-size: .86rem; line-height: 1.6; margin-bottom: 20px; color: #166534; }
...
<body>
    <div class="wrapper">
        <div class="header">
            <h1>Valid ID Approved</h1>
            <p>Municipal Social Welfare and Development Office</p>
        </div>
        <div class="body">
            <div class="title">Hello, {{ $user->full_name }}!</div>
            <div class="msg">
                Good news! Your uploaded valid ID has been reviewed and <strong>approved</strong> by the MSWDO admin for <strong>{{ $user->municipality }}</strong>.
                Your account is now active and you may login to the member portal.
            </div>
            <div class="box">
                <strong>Account Status:</strong> Active<br>
                <strong>Municipality:</strong> {{ $user->municipality }}<br>
                <strong>Approved on:</strong> {{ now()->setTimezone('Asia/Manila')->format('M d, Y h:i A') }}
            </div>
            <a href="{{ $loginUrl }}" class="cta">Login to MSWDO Portal</a>
        </div>
        <div class="foot">
